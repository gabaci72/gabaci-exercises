...
python_file = r"c:\Users\PC\Desktop\Algorithm\python2pseudo\py_file.py"
...
"""def p2file(to_file):
    ...
    with open(py_file + '_pseudo.txt', 'w') as writer:
        writer.write("\n".join(to_file))"""
def p2file(to_file):
    output_directory = os.path.dirname(os.path.abspath(python_file))
    output_filename = os.path.join(output_directory, os.path.splitext(os.path.basename(python_file))[0] + '_pseudo.txt')
...
def main():
    print("Trying to open:", python_file)
    try:
